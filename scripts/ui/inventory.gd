## 物品栏UI - Beta v0.15 美术重制版
## 显示收集的物品和数量，Shift+Tab切换
## v0.15: 美术重制 - 精美面板纹理+阴影文字+图标美化
extends CanvasLayer
...
# 输入消耗标志（防止按住Shift+Tab反复触发）
var _shift_tab_consumed: bool = false
...
func process_input() -> void:
        ...
        if Input.is_action_just_pressed("menu_tab") and Input.is_action_pressed("menu_shift"):
                if not _shift_tab_consumed:
                        _shift_tab_consumed = true
                        close()
        else:
                _shift_tab_consumed = false
