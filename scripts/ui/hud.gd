...
# v0.13: 游侠连击点数指示器
var combo_dots: Array = []  # 5个ColorRect
...
func _build_player_hud() -> void:
        ...
        if is_ranger_mode:
                war_cry_indicator.color = Color(0.6, 0.3, 1.0, 0.0)  # 紫色(影步)
        else:
                war_cry_indicator.color = Color(1, 0.7, 0.2, 0.0)  # 橙色(战吼)
        add_child(war_cry_indicator)

        # v0.13: 游侠连击点数指示器（5个圆点）
        if is_ranger_mode:
                for i in range(5):
                        var dot = ColorRect.new()
                        dot.size = Vector2(10, 10)
                        dot.position = Vector2(10 + i * 24, 49)
                        dot.color = Color(0.3, 0.15, 0.4, 0.4)  # 暗紫色(空)
                        add_child(dot)
...
func update_rage(current: float, maximum: float) -> void:
        if is_ranger_mode:
                # 游侠模式：更新连击点数
                var cp: int = int(current / 20.0)  # rage=CP*20
...
func update_combo_points(points: int) -> void:
        ...
        for i in range(combo_dots.size()):
                var dot: ColorRect = combo_dots[i]
                if i < points:
                        # 点亮的点
                        if i >= 4:
                                dot.color = Color(1.0, 0.3, 0.8, 1.0) if int(frame_count / 6) % 2 == 0 else Color(0.8, 0.2, 0.6, 1.0)
                        elif i >= 2:
                                dot.color = Color(0.7, 0.4, 1.0, 1.0)
                        else:
                                dot.color = Color(0.5, 0.7, 1.0, 1.0)
                else:
                        # 空的点
                        dot.color = Color(0.3, 0.15, 0.4, 0.4)
...
func show_war_cry_buff(active: bool, timer: float = 0) -> void:
        ...
        if war_cry_indicator:
                if active:
                        if is_ranger_mode:
                                war_cry_indicator.color = Color(0.6, 0.3, 1.0, 0.8)  # 紫色(影步)
                                var ratio: float = timer / 2.0  # 影步2秒
                                war_cry_indicator.size.x = 40 * ratio
                        else:
                                war_cry_indicator.color = Color(1, 0.7, 0.2, 0.8)  # 橙色(战吼)
                                var ratio: float = timer / 8.0  # 战吼8秒
                                war_cry_indicator.size.x = 40 * ratio
                else:
                        if is_ranger_mode:
