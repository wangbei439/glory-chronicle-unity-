...
## v0.21: 实际装备图标TextureRect + 装备变更时通知角色更新视觉
extends CanvasLayer
...
# 输入冷却（防止按住W/S/Up/Down连跳）
var _nav_cooldowns: Dictionary = {}
const NAV_COOLDOWN_TIME: float = 0.18

# === 外部引用 ===
var drop_system: Node2D = null
var crafting_system: Node2D = null
var player_controller: Node2D = null  # v0.21: 角色控制器引用（用于通知装备变更）

# v0.21: 装备图标路径
const EQUIP_ICON_PATHS: Dictionary = {
	...
	"leather_vest": "res://assets/sprites/equipment/leather_vest.png",
	"iron_plate": "res://assets/sprites/equipment/iron_plate.png",
	"crystal_mail": "res://assets/sprites/equipment/crystal_mail.png",
	"beetle_bulwark": "res://assets/sprites/equipment/beetle_bulwark.png",
	"vein_holy_garb": "res://assets/sprites/equipment/vein_holy_garb.png",
}
...
func build() -> void:
	...
	for i in range(4):
		...
		# v0.21: 装备图标TextureRect叠加在ColorRect上方
		var icon_tex = TextureRect.new()
		icon_tex.size = Vector2(22, 22)
		icon_tex.position = Vector2(124, slot_y + 11)
		icon_tex.stretch_mode = TextureRect.STRETCH_SCALE
		icon_tex.visible = false
		add_child(icon_tex)
		panel_nodes.append(icon_tex)
		...
		item_slots.append({
			...
			"bg": bg,
			"icon": icon,
			"icon_tex": icon_tex,  # v0.21: TextureRect图标
...
func process_input(delta: float) -> void:
	...
	# 更新冷却
	for key in _nav_cooldowns.keys():
		if _nav_cooldowns[key] > 0:
			_nav_cooldowns[key] -= delta
			if _nav_cooldowns[key] <= 0:
				_nav_cooldowns.erase(key)

	# E键关闭
	if Input.is_action_just_pressed("menu_equip"):
		if not _nav_cooldowns.has("e"):
			_nav_cooldowns["e"] = NAV_COOLDOWN_TIME
			close()
		return
...
func _is_nav_just_pressed(direction: String) -> bool:
	"""带冷却的导航键检测：使用is_action_just_pressed确保只响应一次"""
	var is_pressed: bool = false
	match direction:
		"up":
			is_pressed = Input.is_action_just_pressed("menu_up") or Input.is_action_just_pressed("jump")
		"down":
	...
	# 冷却保护
	if _nav_cooldowns.has(direction) and _nav_cooldowns[direction] > 0:
		return false
	
	_nav_cooldowns[direction] = NAV_COOLDOWN_TIME
	return true
	...
	for i in range(4):
		...
		if current_tab < 2:
			...
			if i < item_keys.size():
				...
				# v0.21: 加载实际装备图标
				if slot.has("icon_tex"):
					var icon_path: String = EQUIP_ICON_PATHS.get(key, "")
					if icon_path != "":
						var tex = load(icon_path)
						if tex:
							slot["icon_tex"].texture = tex
							slot["icon_tex"].visible = true
						else:
							slot["icon_tex"].visible = false
					else:
						slot["icon_tex"].visible = false
			...
			if i < CRAFT_RECIPES.size():
				...
				# v0.21: 加载打造装备图标
				if slot.has("icon_tex"):
					var craft_icon_path: String = EQUIP_ICON_PATHS.get(recipe["id"], "")
					if craft_icon_path != "":
						var tex = load(craft_icon_path)
						if tex:
							slot["icon_tex"].texture = tex
							slot["icon_tex"].visible = true
						else:
...
func _notify_player_equipment_changed() -> void:
	## v0.21: 通知角色控制器更新装备视觉
	if player_controller and player_controller.has_method("update_equipment_visuals"):
		player_controller.update_equipment_visuals()
