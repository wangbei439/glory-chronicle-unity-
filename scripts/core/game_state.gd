...
# === v0.13：职业选择 ===
var selected_class: String = "warrior"  # "warrior" / "ranger" / "mage"
...
func go_to_level(level_name: String) -> void:
        if SCENES.has(level_name):
                ...
                if tree and tree.current_scene:
                        ...
                        # 淡出动画
                        var tween = tree.current_scene.create_tween()
                        tween.tween_property(fade, "color:a", 1.0, 0.3)
                        var scene_path: String = SCENES[level_name]
                        tween.tween_callback(func():
                                var t = get_tree()
                                if t:
                                        t.change_scene_to_file(scene_path)
                                if canvas and is_instance_valid(canvas):
                                        canvas.queue_free()
                        )
                else:
                        if tree:
                                tree.change_scene_to_file(SCENES[level_name])
...
func go_to_title() -> void:
        ...
        if tree and tree.current_scene:
                ...
                var tween = tree.current_scene.create_tween()
                tween.tween_property(fade, "color:a", 1.0, 0.3)
                var scene_path: String = SCENES["title"]
                tween.tween_callback(func():
                        var t = get_tree()
                        if t:
                                t.change_scene_to_file(scene_path)
                ...
                if tree:
                        tree.change_scene_to_file(SCENES["title"])
