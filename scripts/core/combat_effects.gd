...
func spawn_slash_arc(pos: Vector2, facing: float, attack_type: String = "light", combo_step: int = 1) -> void:
        """斩击弧线特效 - v0.19新增，让不同攻击类型有明显视觉区分
        light: 白色短弧, light2/3: 渐亮黄色中弧, heavy: 红色大弧, finisher: 紫色巨弧"""
        var arc_color: Color = Color(1, 1, 0.9, 0.8)  # 默认白
        var arc_width: float = 2.0
        var arc_length: int = 6  # 粒子数量
        var arc_size: float = 3.0
        
        match attack_type:
                "heavy":
                        arc_color = Color(1, 0.5, 0.15, 0.9)
                        arc_width = 3.0
                        arc_length = 10
                        arc_size = 4.0
                "finisher":
                        arc_color = Color(0.8, 0.5, 1.0, 0.95)
                        arc_width = 4.0
                        arc_length = 14
                        arc_size = 5.0
                _:
                        # 轻攻根据步骤递增
                        if combo_step >= 3:
                                arc_color = Color(1, 0.85, 0.3, 0.85)
                                arc_width = 2.5
                                arc_length = 8
                                arc_size = 3.5
                        elif combo_step >= 2:
                                arc_color = Color(1, 0.95, 0.6, 0.8)
                                arc_width = 2.0
                                arc_length = 7
                                arc_size = 3.0
        ...
        for i in range(arc_length):
                var t: float = float(i) / float(arc_length)
                var angle: float = lerp(-PI * 0.6, PI * 0.3, t)  # 从上到下的弧线
                var radius: float = lerp(20, 45, t)  # 弧线半径递增
                var px: float = pos.x + cos(angle) * radius * facing
                var py: float = pos.y + sin(angle) * radius - 25
                
                var p = ColorRect.new()
                p.size = Vector2(arc_size, arc_size)
                p.position = Vector2(px - arc_size * 0.5, py - arc_size * 0.5)
                # 颜色沿弧线渐变：中心最亮
                var brightness: float = 1.0 - abs(t - 0.5) * 1.2
                var c: Color = Color(
                        min(1, arc_color.r * brightness + 0.3),
                        min(1, arc_color.g * brightness + 0.2),
                        min(1, arc_color.b * brightness + 0.1),
                        arc_color.a * brightness
                )
                p.color = c
                add_child(p)
                particles.append({
                        ...
                        "life": 0.12 + t * 0.08,
                        "max_life": 0.2,
                        "vel": Vector2(facing * 40 * (1 - t), -30 * t),
                        "gravity": 50.0,
                        "fade": true,
                        "type": "slash",
                        "trail": [],
