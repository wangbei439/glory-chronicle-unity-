...
func _physics_process(delta: float) -> void:
        ...
        # 使用is_key_just_pressed防止按住时反复toggle
        ...
        # 更新HUD（性能优化：只在变化时更新）
        if abs(player.hp - hud._last_hp) > 0.01 or abs(player.max_hp - hud._last_max_hp) > 0.01:
                hud.update_player_hp(player.hp, player.max_hp)
        if abs(player.rage - hud._last_rage) > 0.01 or abs(player.max_rage - hud._last_max_rage) > 0.01:
                hud.update_rage(player.rage, player.max_rage)
        ...
        # 矿石计数（性能优化：只在数值变化时更新）
        var current_ore: int = drop_system.ore_fragments
...
func _check_player_vs_bats() -> void:
        ...
        for i in range(bats.size()):
                ...
                if dist_x < 65 and dist_y < 50:
                        ...
                        bat.take_damage(dmg)
                        player.mark_hit_dealt()
                        var hit_pos: Vector2 = (player.pos + bat.pos) / 2 + Vector2(0, -20)
                        # v0.19: 根据攻击类型/连击步骤差异化视觉反馈
                        var is_finisher: bool = player.is_combo_finisher if player.has_method("get") else false
                        var is_heavy: bool = player.is_heavy_attack if player.has_method("get") else dmg >= 20
                        var combo_step: int = player.current_combo_step if player.has_method("get") else 0
                ...
                for rock_data in trap["rocks"]:
                        ...
                        if node and is_instance_valid(node):
                                ...
                                if rock_dist < 20 and abs(player.pos.y - 30 - node.position.y) < 30:
                                        var dmg: float = _get_skill_reduced_damage(8.0) * (1.0 - (equipment.get_defense_bonus() if equipment else 0.0))
