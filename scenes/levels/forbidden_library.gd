## 禁术图书馆关卡 - Beta v0.19
## 设计文档§3.1：元素区域，服务"环境反应"
## 3房间滚动关卡，含魔法阵(触碰持续伤害)和魔法陷阱(定时喷发)
## 关底传送门：禁书守护者
extends Node2D
...
# 浮书障碍
var floating_books: Array = []  # {x, y, base_y, node, phase, speed}
...
# === 环境反应机制 ===
# 魔法阵（持续伤害区域，替代熔岩池）
var magic_circles: Array = []  # {x, width, node}
var magic_damage_timer: float = 0.0
const MAGIC_DAMAGE: float = 5.0  # 每次触碰伤害
const MAGIC_DAMAGE_INTERVAL: float = 0.8  # 伤害间隔

# 魔法陷阱（定时喷发，替代间歇泉）
var magic_traps: Array = []  # {x, y, timer, interval, active, duration, nodes}
var magic_trap_timer: float = 0.0
...
func _build_scene() -> void:
        # === 视差背景层（3个房间拼接）===
        for i in range(3):
                var far_tex = load("res://assets/sprites/background/parallax_mine_mid.png")
                if far_tex:
                        ...
                        parallax.modulate = Color(0.7, 0.5, 0.9, 0.5)
                        add_child(parallax)
        ...
        for i in range(3):
                ...
                if bg_tex:
                        ...
                        bg.modulate = Color(0.6, 0.5, 0.8, 1.0)
                        ...
                        bg2.color = Color(0.08, 0.04, 0.12, 1.0)
                        add_child(bg2)
        ...
        for i in range(1, 3):
                ...
                pillar_l.color = Color(0.12, 0.08, 0.18, 0.6)
                add_child(pillar_l)
                var cap_l = ColorRect.new()
                cap_l.position = Vector2(i * 640 - 15, 55)
                cap_l.size = Vector2(30, 8)
                cap_l.color = Color(0.25, 0.12, 0.35, 0.7)
                add_child(cap_l)
                # 拱门顶部符文装饰
                var rune_glow = ColorRect.new()
                rune_glow.position = Vector2(i * 640 - 4, 56)
                rune_glow.size = Vector2(8, 6)
                rune_glow.color = Color(0.5, 0.2, 0.9, 0.4)
                add_child(rune_glow)
        ...
        # === 房间2：禁书回廊 ===
        _build_room_forbidden_corridor()

        # === 房间3：封印大厅 ===
        _build_room_seal_hall()
        ...
        add_child(effects)
        effects.setup_ambient("arcane", 12)
        ...
        # 游侠/法师max_hp兼容
        if (GameState.is_ranger() or GameState.selected_class == "mage") and state.has("max_hp"):
        ...
        ore_display.add_theme_color_override("font_color", Color(0.7, 0.5, 0.9, 0.9))
        add_child(ore_display)

        # === 房间名称 ===
        room_label = Label.new()
        room_label.text = "禁术图书馆 - 藏书阁入口"
        room_label.position = Vector2(230, 150)
        room_label.add_theme_font_size_override("font_size", 14)
        room_label.add_theme_color_override("font_color", Color(0.6, 0.4, 0.9, 0.0))
        ...
        var skill1_text: String
        var skill2_text: String
        if GameState.selected_class == "mage":
                guard_text = "L:魔盾"
                skill1_text = "U:闪现"
                skill2_text = "I:暴风雪"
...
func _build_ground() -> void:
        ...
        ground_top.color = Color(0.35, 0.25, 0.45, 0.9)
        add_child(ground_top)

        # 地面主体（覆盖3个房间）- 使用石砖纹理
        var ground_tex = load("res://assets/sprites/tiles/ground_stone_32.png")
        for x in range(0, 1920, 32):
                if ground_tex:
                        ...
                        tile_fallback.color = Color(0.1, 0.06, 0.15, 0.7)
        ...
        for x in range(0, 1920, 40):
                if randf() < 0.3:
                        ...
                        crack.color = Color(0.5, 0.2, 0.8, 0.4)
                        add_child(crack)
...
func _build_environment_decor() -> void:
        ...
        for cpos in crystal_positions:
                ...
                crystal.color = Color(0.3, 0.15, 0.7, 0.7)
                add_child(crystal)
                # 符文高光
                var crystal_hl = ColorRect.new()
                crystal_hl.position = cpos + Vector2(1, 0)
                crystal_hl.size = Vector2(2, 6)
                crystal_hl.color = Color(0.6, 0.3, 1.0, 0.5)
                add_child(crystal_hl)
                # 符文光晕
                var crystal_glow = ColorRect.new()
                crystal_glow.position = cpos + Vector2(-3, 4)
                crystal_glow.size = Vector2(12, 6)
                crystal_glow.color = Color(0.4, 0.15, 0.8, 0.12)
        ...
        for spos in chain_positions:
                var ch_length: float = randf_range(20, 40)
                var ch_width: float = randf_range(3, 6)
                # 锁链主体
                var chain = ColorRect.new()
                chain.position = spos
                chain.size = Vector2(ch_width, ch_length)
                chain.color = Color(0.15, 0.1, 0.2, 0.7)
                add_child(chain)
                # 锁链环扣
                var ch_ring = ColorRect.new()
                ch_ring.position = spos + Vector2(ch_width / 2 - 2, ch_length)
                ch_ring.size = Vector2(4, 3)
                ch_ring.color = Color(0.25, 0.15, 0.3, 0.8)
                add_child(ch_ring)
                # 锁链末端微光
                var ch_glow = ColorRect.new()
                ch_glow.position = spos + Vector2(ch_width / 2 - 3, ch_length + 2)
                ch_glow.size = Vector2(6, 4)
                ch_glow.color = Color(0.4, 0.2, 0.8, 0.1)
                add_child(ch_glow)
        ...
        for _e in range(20):
                ...
                if randf() < 0.5:
                        ember.color = Color(0.5, 0.2, 1.0, randf_range(0.2, 0.5))
                else:
                        ember.color = Color(0.2, 0.5, 1.0, randf_range(0.2, 0.4))
                add_child(ember)

func _build_room_entrance() -> void:
        """房间1：藏书阁入口 - 咒缚学者+暗蝠+魔法阵"""
        # 平台
        ...
        # 魔法阵（地面上的持续伤害区域）
        _add_magic_circle(180, 60)
        _add_magic_circle(420, 50)

        # 魔法陷阱
        _add_magic_trap(350, GROUND_Y, 7.0)
        _add_magic_trap(530, GROUND_Y, 9.0)
...
func _build_room_forbidden_corridor() -> void:
        """房间2：禁书回廊 - 密集敌人+浮书+魔法陷阱"""
        # 平台
        ...
        # 魔法阵
        _add_magic_circle(700, 80)
        _add_magic_circle(1000, 70)
        _add_magic_circle(1180, 50)

        # 魔法陷阱
        _add_magic_trap(760, GROUND_Y, 6.0)
        _add_magic_trap(950, GROUND_Y, 8.0)
        _add_magic_trap(1150, GROUND_Y, 5.0)

        # 浮书障碍
        _spawn_floating_book(820, 180, 1.5)
        _spawn_floating_book(1050, 160, 1.2)
        _spawn_floating_book(1230, 200, 1.8)
...
func _build_room_seal_hall() -> void:
        """房间3：封印大厅 - 强敌+暗蝠+传送门到Boss战"""
        # 平台
        ...
        # 魔法阵
        _add_magic_circle(1400, 60)
        _add_magic_circle(1650, 50)

        # 魔法陷阱
        _add_magic_trap(1500, GROUND_Y, 7.0)
        _add_magic_trap(1700, GROUND_Y, 5.5)

        # 封印门装饰
        var gate_arch = ColorRect.new()
        gate_arch.position = Vector2(1830, 220)
        gate_arch.size = Vector2(60, 110)
        gate_arch.color = Color(0.15, 0.08, 0.25, 0.95)
        add_child(gate_arch)
        var gate_top = ColorRect.new()
        gate_top.position = Vector2(1825, 215)
        gate_top.size = Vector2(70, 8)
        gate_top.color = Color(0.3, 0.12, 0.5, 0.8)
        add_child(gate_top)
        # 门上发光符文（紫色光晕）
        var gate_glow = ColorRect.new()
        gate_glow.position = Vector2(1846, 246)
        gate_glow.size = Vector2(28, 28)
        gate_glow.color = Color(0.5, 0.2, 0.9, 0.6)
        add_child(gate_glow)
        # 左侧魔光碗
        var bowl_l = ColorRect.new()
        bowl_l.position = Vector2(1830, 270)
        bowl_l.size = Vector2(4, 6)
        bowl_l.color = Color(0.6, 0.2, 1.0, 0.7)
        add_child(bowl_l)
        # 右侧魔光碗
        var bowl_r = ColorRect.new()
        bowl_r.position = Vector2(1886, 270)
        bowl_r.size = Vector2(4, 6)
        bowl_r.color = Color(0.6, 0.2, 1.0, 0.7)
        add_child(bowl_r)
        ...
        # 蝙蝠
        _spawn_bat(Vector2(1550, 180), 150)
...
func _build_platform(x: float, y: float, width: float) -> void:
        ...
        support2.color = Color(0.15, 0.08, 0.22, 0.6)
        add_child(support2)
        # 右柱高光
        var support2_hl = ColorRect.new()
        support2_hl.position = Vector2(x + width - 1, y + 6)
        support2_hl.size = Vector2(1, GROUND_Y - y - 6)
        support2_hl.color = Color(0.3, 0.15, 0.4, 0.7)
        add_child(support2_hl)

func _add_magic_circle(x: float, width: float) -> void:
        """创建地面魔法阵（替代熔岩池）"""
        # 外层发光（紫色动画用）
        var outer_glow = ColorRect.new()
        outer_glow.position = Vector2(x - 4, 324)
        outer_glow.size = Vector2(width + 8, 10)
        outer_glow.color = Color(0.5, 0.2, 0.9, 0.2)
        add_child(outer_glow)

        # 主阵体（加高到6px）
        var circle_node = ColorRect.new()
        circle_node.position = Vector2(x, 326)
        circle_node.size = Vector2(width, 6)
        circle_node.color = Color(0.5, 0.15, 0.85, 0.8)
        ...
        bright_core.color = Color(0.3, 0.7, 1.0, 0.6)
        ...
        for _b in range(3):
                ...
                rune.position = Vector2(bx, 327 + randf_range(0, 3))
                rune.size = Vector2(2, 2)
                rune.color = Color(0.6, 0.3, 1.0, 0.7)
                add_child(rune)

        magic_circles.append({"x": x, "width": width, "node": circle_node})

func _add_magic_trap(x: float, y: float, interval: float) -> void:
        """创建魔法陷阱（替代间歇泉）"""
        # 陷阱口标记（符文圈）
        var vent = ColorRect.new()
        vent.position = Vector2(x - 8, y - 4)
        vent.size = Vector2(16, 4)
        vent.color = Color(0.3, 0.15, 0.5, 0.6)
        add_child(vent)
...
func _spawn_floating_book(x: float, y: float, speed: float) -> void:
        """创建浮书障碍"""
        var book_node = ColorRect.new()
        book_node.position = Vector2(x, y)
        book_node.size = Vector2(16, 12)
        book_node.color = Color(0.3, 0.15, 0.5, 0.8)
        add_child(book_node)
        # 书页高光
        var book_page = ColorRect.new()
        book_page.position = Vector2(x + 2, y + 2)
        book_page.size = Vector2(12, 8)
        book_page.color = Color(0.5, 0.3, 0.7, 0.5)
        add_child(book_page)
        # 奥术光晕
        var book_glow = ColorRect.new()
        book_glow.position = Vector2(x - 2, y - 2)
        book_glow.size = Vector2(20, 16)
        book_glow.color = Color(0.4, 0.2, 0.8, 0.1)
        add_child(book_glow)

        floating_books.append({
                "x": x,
                "y": y,
                "base_y": y,
                "node": book_node,
...
func _build_portal() -> void:
        # 紫色光晕
        var portal_purple_glow = ColorRect.new()
        portal_purple_glow.size = Vector2(20, 58)
        portal_purple_glow.position = portal_pos + Vector2(-4, -54)
        portal_purple_glow.color = Color(0.5, 0.2, 0.9, 0.15)
        add_child(portal_purple_glow)
        ...
        portal_label.add_theme_color_override("font_color", Color(0.6, 0.3, 1.0, 0.7))
...
func _physics_process(delta: float) -> void:
        ...
        # === 环境反应：魔法阵 ===
        _process_magic_circles(delta)

        # === 环境反应：魔法陷阱 ===
        _process_magic_traps(delta)

        # === 浮书障碍 ===
        _process_floating_books(delta)
        ...
        if room_label_timer > 0:
                ...
                room_label.add_theme_color_override("font_color", Color(0.6, 0.4, 0.9, alpha))
        ...
        # 截图
        if frame_count == 120:
                _take_screenshot("legend_library_level.png")

        # 自动演示/退出
        if auto_demo:
                _run_demo()
        if auto_quit_frame > 0 and frame_count >= auto_quit_frame:
                _take_screenshot("legend_library_auto.png")
                get_tree().quit()
        ...
        # 传送门闪烁（紫色/青色交替）
        if portal:
                var t: float = frame_count * 0.08
                portal.color = Color(0.5 + 0.2 * sin(t), 0.2, 0.9 - 0.2 * sin(t * 1.3), 0.3 + 0.3 * sin(t))

# === 环境反应：魔法阵 ===
func _process_magic_circles(delta: float) -> void:
        magic_damage_timer -= delta
        if magic_damage_timer > 0:
                return
        ...
        for circle in magic_circles:
                if player.pos.x >= circle["x"] and player.pos.x <= circle["x"] + circle["width"]:
                        if player.invincible_timer <= 0:
                                var dmg: float = MAGIC_DAMAGE * (1.0 - (skill_tree.get_defense_bonus() if skill_tree else 0.0)) * (1.0 - (equipment.get_defense_bonus() if equipment else 0.0))
                                player.take_damage(dmg, Vector2(randf_range(-2, 2), -3))
                                hud.spawn_damage_number(player.pos + Vector2(0, -40), dmg, false)
                                hud.show_perfect("MAGIC!", Color(0.5, 0.2, 0.9))
                                audio.play("hurt", 0.4)
                                magic_damage_timer = MAGIC_DAMAGE_INTERVAL
                        break

        # 魔法阵闪烁动画（紫/青交替）
        for circle in magic_circles:
                if circle["node"] and is_instance_valid(circle["node"]):
                        var intensity: float = 0.6 + 0.4 * sin(frame_count * 0.05)
                        var phase: float = sin(frame_count * 0.03)
                        circle["node"].color = Color(0.5 * intensity, 0.15 * intensity, 0.85 * intensity + 0.1 * phase, 0.8)

# === 环境反应：魔法陷阱 ===
func _process_magic_traps(delta: float) -> void:
        for trap in magic_traps:
                if trap["active"]:
                        trap["duration"] -= delta
                        # 喷发伤害判定
                        if not trap["dealt_damage"]:
                                var dist_x: float = abs(player.pos.x - trap["x"])
                                ...
                                if dist_x < 30 and dist_y < 40 and player.invincible_timer <= 0:
                                        ...
                                        hud.spawn_damage_number(player.pos + Vector2(0, -40), dmg, false)
                                        hud.show_perfect("ARCANE!", Color(0.4, 0.2, 0.9))
                                        effects.start_shake(3.0, 8.0)
                                        audio.play("rock_fall")
                                        trap["dealt_damage"] = true
                        ...
                        trap["timer"] -= delta
                        # 预警：喷发前1秒闪烁（紫色）
                        if trap["timer"] < 1.0 and trap["timer"] > 0:
                                if trap["vent"] and is_instance_valid(trap["vent"]):
                                        trap["vent"].color = Color(0.6, 0.2, 1.0, 0.5 + 0.5 * sin(trap["timer"] * 15))
                        if trap["timer"] <= 0:
                                # 喷发！
                                trap["active"] = true
                                trap["duration"] = 1.0
                                trap["dealt_damage"] = false
                                ...
                                for j in range(5):
                                        ...
                                        var p_pos: Vector2 = Vector2(trap["x"] + randf_range(-8, 8), GROUND_Y - 5)
                                        particle.position = p_pos
                                        if randf() < 0.5:
                                                particle.color = Color(0.5, 0.2, 1.0, 0.9)
                                        else:
                                                particle.color = Color(0.2, 0.5, 1.0, 0.9)
                                        add_child(particle)
                                        trap["nodes"].append({
                                                "node": particle,
                                                "pos": p_pos,
...
# === 浮书障碍 ===
func _process_floating_books(delta: float) -> void:
        for book in floating_books:
                book["phase"] += book["speed"] * delta
                var new_y: float = book["base_y"] + sin(book["phase"]) * 20
                book["y"] = new_y
                if book["node"] and is_instance_valid(book["node"]):
                        book["node"].position = Vector2(book["x"], new_y)
                if book["page_node"] and is_instance_valid(book["page_node"]):
                        book["page_node"].position = Vector2(book["x"] + 2, new_y + 2)
                if book["glow_node"] and is_instance_valid(book["glow_node"]):
                        book["glow_node"].position = Vector2(book["x"] - 2, new_y - 2)
                        # 光晕闪烁
                        var glow_intensity: float = 0.08 + 0.06 * sin(frame_count * 0.06 + book["phase"])
                        book["glow_node"].color = Color(0.4, 0.2, 0.8, glow_intensity)

                # 碰撞检测：浮书伤害玩家
                var dist_x: float = abs(player.pos.x - (book["x"] + 8))
                var dist_y: float = abs(player.pos.y - 30 - (new_y + 6))
                if dist_x < 24 and dist_y < 22 and player.invincible_timer <= 0:
                        ...
                        player.take_damage(dmg, Vector2(randf_range(-2, 2), -3))
                        hud.spawn_damage_number(player.pos + Vector2(0, -40), dmg, false)
                        hud.show_perfect("BOOK!", Color(0.5, 0.3, 0.9))
                        audio.play("hurt", 0.3)
...
func _check_room_change() -> void:
        ...
        if new_room != current_room:
                ...
                match current_room:
                        0: _show_room_name("禁术图书馆 - 藏书阁入口")
                        1: _show_room_name("禁术图书馆 - 禁书回廊")
                        2: _show_room_name("禁术图书馆 - 封印大厅")

func _check_portal() -> void:
        var dist_to_portal = abs(player.pos.x - portal_pos.x)
        if dist_to_portal < 25:
                audio.play("portal")
                GameState.mark_level_cleared("library")
                SaveSystem.save_game()
                GameState.go_to_level("library_boss")
...
func _check_player_vs_bats() -> void:
        ...
        for i in range(bats.size()):
                ...
                if dist_x < 65 and dist_y < 50:
                        ...
                        bat.take_damage(dmg)
                        player.mark_hit_dealt()
                        effects.spawn_hit_spark((player.pos + bat.pos) / 2, Color(0.5, 0.3, 0.9))
...
func _on_enemy_died(pos: Vector2) -> void:
        effects.spawn_hit_spark(pos, Color(0.5, 0.3, 0.9))
        player.rage = min(player.max_rage, player.rage + _get_skill_boosted_rage(15.0))
        hud.show_perfect("+15 RAGE", Color(0.5, 0.3, 0.9)); audio.play("enemy_die")
        GameState.total_kills += 1
        drop_system.spawn_drop(pos, "wraith", GROUND_Y)
        # v0.17: 图书馆小怪稀有掉落（禁忌卷轴12%）
        if randf() < 0.12:
                GameState.crafting_materials["forbidden_scroll"] = int(GameState.crafting_materials.get("forbidden_scroll", 0)) + 1
                hud.show_perfect("+1 禁忌卷轴!", Color(0.6, 0.3, 1.0))

func _on_bat_hit_player(damage: float, knockback: Vector2) -> void: pass
func _on_bat_died(pos: Vector2) -> void:
        effects.spawn_hit_spark(pos, Color(0.6, 0.3, 1.0))
        player.rage = min(player.max_rage, player.rage + _get_skill_boosted_rage(10.0))
        hud.show_perfect("+10 RAGE", Color(0.6, 0.3, 1.0)); audio.play("enemy_die")
        GameState.total_kills += 1
        drop_system.spawn_drop(pos, "bat", GROUND_Y)
        # v0.17: 图书馆暗蝠稀有掉落（禁忌卷轴10%）
        if randf() < 0.10:
                GameState.crafting_materials["forbidden_scroll"] = int(GameState.crafting_materials.get("forbidden_scroll", 0)) + 1
                hud.show_perfect("+1 禁忌卷轴!", Color(0.6, 0.3, 1.0))
...
func _save_state() -> void:
        ...
        GameState.current_level = "library"
...
func _on_game_over_retry() -> void:
        ...
        player.hit_count = 0
        if audio: audio.play("level_up", 0.5)
